#!/usr/bin/env python3
"""
scan_for_gog.py - Scans folders for executables and creates shortcuts config.
Filter list for unwanted executables, allowing for duplicates if specified.
"""
import json
import sys
...
def parse_args():
    ...
    parser = ArgumentParser(description="Scan folder for gog executables and create shortcuts config.")
...
def scan_folder_for_exes(folder_path, config_path=None, allow_duplicates=False):
    ...
    for exe in folder.rglob('*.exe'):
        ...
        # specific_names = ['doom', 'boom', 'heretic', 'hexen', 'strife']
        # if not any(term in exe.name.lower() for term in specific_names):
            continue
        
        # Skip common junk
        if exe.name.lower().startswith(('_', 'gog_', 'unins', 'autorun', 'update', 'crash', 'config', 'setup', 'dos')):
            continue
    ...
    if found_count == 0:
        print("No gog executables found!")
        return False
    ...
    print(f"✓ Found {found_count} gog executables")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: scan_for_gog.py <folder_path> [config_path]")
        sys.exit(1)
